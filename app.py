...
def initialize_session_state():
    ...
    if 'mistral_client' not in st.session_state: # Keep this for existing Mistral setup
        st.session_state.mistral_client = None
    if 'ollama_client' not in st.session_state: # Prepare for direct Ollama client in session_state if needed later
    ...
    # Initialize OllamaClient and add to providers
    if 'ollama' not in st.session_state.providers:
        try:
            st.session_state.providers['ollama'] = OllamaClient() # Default URL http://localhost:11434/api
            # Optionally, could add a check here:
            # if not st.session_state.providers['ollama'].list_models(): # Example check
            #     print("Warning: Ollama client initialized but could not connect or find models.")
            # else:
            #     print("Ollama client initialized and connected.")
        except Exception as e:
            print(f"Failed to initialize OllamaClient: {e}")
            st.session_state.providers['ollama'] = None # Ensure it's None if init fails
