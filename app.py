...
def take_screenshot_and_analyze(livestream_placeholder=None):
    """Take screenshot, analyze with element detection, and update livestream."""
    try:
        ...
        if livestream_placeholder and screenshot_path:
            livestream_placeholder.image(screenshot_path, caption="Live View - Current", use_column_width=True)
        ...
        if livestream_placeholder and annotated_image_path:
            livestream_placeholder.image(annotated_image_path, caption="Live View - Annotated", use_column_width=True)
...
def execute_automation_step(user_objective, livestream_placeholder=None):
    ...
    try:
        ...
        annotated_image_path = take_screenshot_and_analyze(livestream_placeholder)
...
def update_livestream_display(image_placeholder):
    """Update the livestream display with a current screenshot or status message."""
    try:
        if st.session_state.browser and st.session_state.browser.driver:
            screenshot_path = st.session_state.browser.take_screenshot(filename_prefix="livestream_")
            if screenshot_path:
                image_placeholder.image(screenshot_path, caption="Live Browser View", use_column_width=True)
            else:
                image_placeholder.info("Could not capture screenshot for livestream.")
        else:
            image_placeholder.info("Browser not running or not started. Start the browser from the sidebar to see the livestream.")
    except Exception as e:
        # image_placeholder.error(f"Error updating livestream: {str(e)}")
        # To avoid flooding with errors if screenshot fails repeatedly, let's use info
        image_placeholder.info(f"Livestream paused due to an issue (e.g. browser closed unexpectedly or page loading): {str(e)}")
...
def main():
    ...
    with left_column:
        ...
        while st.session_state.automation_active and step_count < max_steps:
            ...
            success = execute_automation_step(user_input, livestream_placeholder)
            if not success:
                break
    ...
    # Auto-continue automation if active (within left column)
    if st.session_state.automation_active:
        # This part might need adjustment depending on desired behavior with columns
        # For now, keeping it such that rerun affects the whole page,
        # which should be fine as long as state is managed.
        time.sleep(1) # Short delay before forcing rerun for automation continuation
        st.rerun()
    ...
    with right_column:
        ...
        if st.session_state.browser and st.session_state.browser.driver:
            ...
            if not st.session_state.automation_active:
                # Automation is NOT active, so periodically refresh the view
                update_livestream_display(livestream_placeholder)
                time.sleep(1.5) # Refresh interval
                st.rerun() # Schedule next refresh
            else:
                # Automation IS active. Livestream is updated by execute_automation_step.
                # Call update_livestream_display once to ensure the view is current if
                # automation just started and execute_automation_step hasn't updated the placeholder yet.
                update_livestream_display(livestream_placeholder)
        else:
            # Browser is not active or driver not available
            update_livestream_display(livestream_placeholder) # This will show "Browser not running..."
